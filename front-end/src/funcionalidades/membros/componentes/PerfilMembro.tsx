...
export function PerfilMembro({ membroId }: { membroId: string }) {
    const { membros, carregando: carregandoMembros, erro: erroMembros, recarregar } = usarMembros();
        ...
        <div className="w-full space-y-10 pb-20 animate-in fade-in duration-500">
            ...
            {carregandoMembros ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 gap-4 bg-card border border-rose-100 rounded-3xl text-center px-4">
                    ...
                    <button onClick={() => recarregar()} className="mt-4 rounded-xl font-bold uppercase text-[10px] tracking-widest px-4 py-2 border border-border">
                        Tentar novamente
                    </button>
                </div>
            ) : !membro ? (
